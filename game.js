...
function rosterCardMarkup(w){
  return `<article class="rosterTradeCard">
    ...
    <div class="rosterTradeArt"><img src="${w.portrait}" alt="${w.name}"></div>
...
function renderDeckView(id){
  ...
  $('#deckHero').innerHTML=`<article class="rosterTradeCard featured"><div class="rosterTradeTop"><span class="rosterChip">${w.era} persona</span><span class="rosterChip overall">Overall ${overallRating(w)}</span></div><div class="rosterTradeArt"><img src="${w.portrait}" alt="${w.name}"></div><div class="rosterTradeName"><h3>${w.name}</h3><p>${w.ai?.style||'Wrestler'}</p></div><div class="rosterTradeStats stats"><span>Power ${w.stats.power}</span><span>Speed ${w.stats.speed}</span><span>Brawling ${w.stats.brawling}</span><span>Technique ${w.stats.technique}</span><span>Ring IQ ${w.stats.ringIQ}</span><span>Resilience ${w.stats.resilience}</span></div></article>`;
