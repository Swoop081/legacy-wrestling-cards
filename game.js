...
function home(){
 ...
 render(`<section class="game-hub"><div class="hub-copy"><div class="tv-kicker">WELCOME TO LPW</div><h1>TAG TEAM <span>GAUNTLET</span></h1><p>Build a team, survive the broadcast and unlock the Founding Twenty.</p><nav class="hub-menu"><button class="hub-option primary live-menu-option" onclick="gauntletLiveHome()"><b>CAREER</b><small>Begin or continue your weekly career.</small></button><button class="hub-option" onclick="classicHome()"><b>CLASSIC GAUNTLET</b><small>One loss ends the run.</small></button><button class="hub-option" onclick="quickMatchMenu()"><b>QUICK MATCH</b><small>Singles and Tag Team exhibition framework.</small></button><button class="hub-option" onclick="collection()"><b>COLLECTION</b><small>Explore the roster and discover every wrestler in LEGACY Pro Wrestling.</small></button><button class="hub-option" onclick="statisticsMenu()"><b>STATISTICS</b><small>Your legacy framework.</small></button><button class="hub-option muted" onclick="optionsMenu()"><b>OPTIONS</b><small>Presentation settings coming soon.</small></button></nav></div><article class="featured-superstar"><div class="live-chip">FEATURED WRESTLER</div>${imageWithFallback(w,'full','art-full','homeFeature')}<div class="featured-lower-third"><small>${w.title}</small><h2>${w.name}</h2><p>${FEATURE_LINES[w.id]||w.signature}</p><button onclick="collectionProfile('${w.id}')">VIEW PROFILE</button></div></article></section>`)
}
...
const _homeV52=home;
home=function(){clearStoryTimer();M=null;overlay.innerHTML='';const w=featuredSuperstar();render(`<section class="game-hub"><div class="hub-copy"><div class="tv-kicker">WELCOME TO LPW</div><h1>TAG TEAM <span>GAUNTLET</span></h1><p>Build a team, survive the broadcast and create your legacy.</p><nav class="hub-menu"><button class="hub-option primary live-menu-option" onclick="gauntletLiveHome()"><b>CAREER</b><small>Begin or continue your weekly career.</small></button><button class="hub-option" onclick="classicHome()"><b>CLASSIC GAUNTLET</b><small>One loss ends the run.</small></button><button class="hub-option tournament-option" onclick="tournamentHome()"><b>GAUNTLET CUP</b><small>Eight-team single-elimination tournament.</small></button><button class="hub-option" onclick="quickMatchMenu()"><b>QUICK MATCH</b><small>Singles and Tag Team exhibitions.</small></button><button class="hub-option" onclick="collection()"><b>COLLECTION</b><small>Explore the roster and discover every wrestler in LEGACY Pro Wrestling.</small></button><button class="hub-option" onclick="achievementMenu()"><b>ACHIEVEMENTS</b><small>Career milestones and challenges.</small></button><button class="hub-option" onclick="statisticsMenu()"><b>STATISTICS</b><small>Your complete match history.</small></button><button class="hub-option" onclick="helpMenu()"><b>HELP & GUIDE</b><small>Rules, decisions and mode explanations.</small></button></nav></div><article class="featured-superstar"><div class="live-chip">FEATURED WRESTLER</div>${imageWithFallback(w,'full','art-full','homeFeature')}<div class="featured-lower-third"><small>${w.title}</small><h2>${w.name}</h2><p>${FEATURE_LINES[w.id]||w.signature}</p><button onclick="collectionProfile('${w.id}')">VIEW PROFILE</button></div></article></section>`)};
...
(function(){
 ...
 function installMenuButton(){
  ...
  b.onclick=()=>window.battleRoyalHome();b.innerHTML='<b>20-PERSON BATTLE ROYAL</b><small>Twenty enter, one survives. Can you outlast the field in the Battle Royal?</small>';
...
render=function(html){
 ...
 if(main){main.querySelectorAll('.lpw-main-brand,.hub-copy > .lpw-brand-logo,.hub-copy > img[src*="lpw-logo"]').forEach(node=>node.remove());}
...
(function(){
 ...
 function rankLabel(c,id){
  if(id===c?.championships?.world)return 'WORLD CHAMPION';
  const i=rankingRows(c).findIndex(row=>row.id===id);
  return i>=0?`POWER RANK #${i+1}`:'UNRANKED';
 }
 function previewRoster(c){
  const pending=c?.pending||{};
  return [c?.active,pending.partner,...(pending.opponents||[pending.opponent])]
   .filter((id,index,list)=>id&&list.indexOf(id)===index)
   .map(liveFounder).filter(Boolean);
 }
 function enhanceMatchPreview(){
  const c=liveLoad(),card=document.querySelector('.live-match-card');
  if(!c||!card)return;
  card.classList.add('lpw-match-card-911');
  const back=[...card.querySelectorAll('button')].find(b=>/CALENDAR/i.test(b.textContent||''));
  if(back)back.classList.add('lpw-match-calendar-back');
  const branding=card.querySelector('.lpw-show-logo,.lpw-ple-title,.lpw904-supercard-title,.supercard-logo');
  if(branding)branding.classList.add('lpw-match-branding');
  const broadcast=[...card.querySelectorAll('button')].find(b=>/BEGIN (MATCH BROADCAST|SUPER ?CARD MATCH)/i.test(b.textContent||''));
  if(broadcast){
   broadcast.classList.add('lpw-broadcast-button-top');
   if(branding)branding.insertAdjacentElement('afterend',broadcast);
  }
  const wrestlers=previewRoster(c),cards=[...card.querySelectorAll('.live-match-lineup > div')];
