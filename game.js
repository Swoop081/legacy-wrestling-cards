...
function renderHand(){
  const f=state.player;$('#hand').innerHTML=f.hand.map((k,i)=>{const c=C[k],selected=state.discardPhase&&state.discardSelected.has(i),p=state.discardPhase?{ok:true,why:selected?'Selected to discard':'Tap to discard'}:playability(k,f),acc=(isReversal(k)||c.counter)?counterAccuracy(c):effectiveAccuracy(c,f,state.cpu),req=state.discardPhase?p.why:p.ok?((isReversal(k)||c.counter)&&state.pendingAttack?`Counters ${C[state.pendingAttack.key].name}`:'Playable now'):p.why,combo=f.combo===c.name?'<div class="combo">COMBO READY · +10% accuracy</div>':c.combo?`<div class="combo">Sets up ${C[c.combo].name}</div>`:'';return `<button class="gameCard type-${String(c.type||'other').toLowerCase().replace(/[^a-z0-9]+/g,'-')} ${p.ok?'':'unplayable'} ${selected?'discardSelected':''} ${c.image?'hasArt':''}" data-card="${i}" ${p.ok?'':'disabled'}>${c.image?`<div class="cardArt" style="background-image:url('${c.image}')"></div>`:''}<h4>${c.name}</h4><div class="requirement">${req}</div>${combo}<div class="cardStats"><div class="cardStat"><span>DMG</span><b>${c.damage||0}</b></div><div class="cardStat"><span>MOM</span><b>${c.momentum>=0?'+':''}${c.momentum||0}</b></div><div class="cardStat"><span>COST</span><b>${c.cost||0}</b></div><div class="cardStat"><span>${(isReversal(k)||c.counter)?'CTR':'ACC'}</span><b>${acc}%</b></div></div></button>`}).join('');
